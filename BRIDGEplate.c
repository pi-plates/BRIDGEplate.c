...
char* CMD(const char* cmd) {
    ...
    // Consume any extra newline that might be in the buffer
    // The device sends response\n\n (response + two newlines)
    // We've already consumed the first \n above, now get the second
    bytes_read = read_serial(g_serial_port, &c, 1);
    if (debug && bytes_read > 0) {
        fprintf(stderr, "[DEBUG] Consumed extra byte: 0x%02X\n", (unsigned char)c);
    }
...
int ADC_getADCall(int addr, double* results) {
    return parseItArray("ADC.getADCall", results, 16, 1, addr);
}

int ADC_getSall(int addr, double* results) {
    return parseItArray("ADC.getSall", results, 8, 1, addr);
}

int ADC_getDall(int addr, double* results) {
    return parseItArray("ADC.getDall", results, 4, 1, addr);
}

int ADC_getIall(int addr, double* results) {
    return parseItArray("ADC.getIall", results, 4, 1, addr);
}
...
int ADC_getSRQ(int addr) { return (int)parseIt("ADC.getSRQ", 1, addr); }
...
int CURRENT_getIall(int addr, double* results) {
    return parseItArray("CURRENT.getIall", results, 8, 1, addr);
}
...
int DAQC_getADCall(int addr, double* results) {
    return parseItArray("DAQC.getADCall", results, 8, 1, addr);
}
...
int DAQC_getSRQ(int addr) { return (int)parseIt("DAQC.getSRQ", 1, addr); }
int DAQC_getEVENTS(int addr) { return (int)parseIt("DAQC.getEVENTS", 1, addr); }
...
int DAQC2_getADCall(int addr, double* results) {
    return parseItArray("DAQC2.getADCall", results, 8, 1, addr);
}
...
int DAQC2_getEVENTS(int addr) { return (int)parseIt("DAQC2.getEVENTS", 1, addr); }
...
int DIGI_getDIN(int addr, int bit) { return (int)parseIt("DIGI.getDIN", 2, addr, bit); }
int DIGI_getDINall(int addr) { return (int)parseIt("DIGI.getDINall", 1, addr); }
double DIGI_getFREQ(int addr, int bit) { return parseIt("DIGI.getFREQ", 2, addr, bit); }
int DIGI_getFREQall(int addr, double* results) {
    return parseItArray("DIGI.getFREQall", results, 8, 1, addr);
}
void DIGI_enableEVENT(int addr, int bit) { parseIt("DIGI.enableEVENT", 2, addr, bit); }
void DIGI_disableEVENT(int addr, int bit) { parseIt("DIGI.disableEVENT", 2, addr, bit); }
int DIGI_getEVENTS(int addr) { return (int)parseIt("DIGI.getEVENTS", 1, addr); }
int DIGI_getSRQ(int addr) { return (int)parseIt("DIGI.getSRQ", 1, addr); }
...
int THERMO_getSRQ(int addr) { return (int)parseIt("THERMO.getSRQ", 1, addr); }
int THERMO_getEVENTS(int addr) { return (int)parseIt("THERMO.getEVENTS", 1, addr); }
