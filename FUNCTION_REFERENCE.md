...
```c
int ADC_getADCall(int addr, double* results);
int ADC_getSall(int addr, double* results);
int ADC_getDall(int addr, double* results);
int ADC_getIall(int addr, double* results);
```

**Description:**  
Read all channels at once. Different functions return different measurement types:
- `getADCall` - All ADC channels (returns 16 values)
- `getSall` - Single-ended measurements (returns 8 values)
- `getDall` - Differential measurements (returns 4 values)
- `getIall` - Current measurements (returns 4 values)

**Parameters:**
- `addr` - Plate address (0-7)
- `results` - Array to store readings (must be large enough: 16 for getADCall, 8 for getSall, 4 for getDall/getIall)
...
**Example:**
```c
double voltages[16];  // 16 values for getADCall
int count = ADC_getADCall(0, voltages);
...
```c
int CURRENT_getIall(int addr, double* results);
```

**Description:**  
Read all current channels at once (returns 8 values).

**Parameters:**
- `addr` - Plate address (0-7)
- `results` - Array to store current readings in mA (must hold 8 values)

**Returns:**  
Number of channels read (8)

**Example:**
```c
double currents[8];
int count = CURRENT_getIall(0, currents);
...
int DAQC_getADCall(int addr, double* results);
```

**Description:**  
Read analog inputs (0-4.096V range, 12-bit resolution). getADCall returns 8 values.

**Parameters:**
- `channel` - ADC channel (0-7)
- `results` - Array to store readings (must hold 8 values for getADCall)
...
int count = DAQC_getADCall(0, all_voltages);
```
...
### Event and Status Functions

```c
int DAQC_getSRQ(int addr);
int DAQC_getEVENTS(int addr);
```

**Description:**  
Get service request status and event notifications.
...
int DAQC2_getADCall(int addr, double* results);  // Returns 8 values
...
The DIGI (Digital I/O) plate provides 8 digital inputs with frequency measurement and event notification capabilities.
...
```c
int DIGI_getDIN(int addr, int bit);
int DIGI_getDINall(int addr);
```

**Description:**  
Read individual digital input bits or all 8 inputs at once.

**Parameters:**
- `bit` - Input bit number (0-7)

**Returns:**  
Single bit state (0 or 1) for getDIN, or 8-bit value for getDINall

**Example:**
```c
// Read single input
int bit0 = DIGI_getDIN(0, 0);
printf("Input 0: %d\n", bit0);

// Read all inputs
int all_inputs = DIGI_getDINall(0);
printf("All inputs: 0x%02X\n", all_inputs);
```

### Frequency Measurement

```c
double DIGI_getFREQ(int addr, int bit);
int DIGI_getFREQall(int addr, double* results);
```

**Description:**  
Measure frequency on digital inputs. getFREQall returns 8 values.

**Parameters:**
- `bit` - Input bit number (0-7)
- `results` - Array to store frequency readings (must hold 8 values)

**Returns:**  
Frequency in Hz for getFREQ, or number of values read for getFREQall

**Example:**
```c
// Measure frequency on single input
double freq = DIGI_getFREQ(0, 0);
printf("Input 0 frequency: %.2f Hz\n", freq);

// Measure all input frequencies
double freqs[8];
int count = DIGI_getFREQall(0, freqs);
for (int i = 0; i < count; i++) {
    printf("Input %d: %.2f Hz\n", i, freqs[i]);
}
```
...
```c
void DIGI_enableEVENT(int addr, int bit);
void DIGI_disableEVENT(int addr, int bit);
int DIGI_getEVENTS(int addr);
int DIGI_getSRQ(int addr);
```

**Description:**  
Enable/disable event notifications for input changes and check for pending events.
...
**Example:**
```c
// Enable event notification for input 3
DIGI_enableEVENT(0, 3);

// Check for pending events
int events = DIGI_getEVENTS(0);
if (events) {
    printf("Events pending: 0x%02X\n", events);
}

// Check service request
int srq = DIGI_getSRQ(0);
```
...
```c
int THERMO_getSRQ(int addr);
int THERMO_getEVENTS(int addr);
```
