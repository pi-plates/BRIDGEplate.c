...
```c
double DAQC2_getTEMP(int addr);
double DAQC2_getTEMP_scale(int addr, char scale);
void DAQC2_setPOWER(int addr, double voltage);
double DAQC2_getPOWER(int addr);
```

### Calibration and Reset

```c
void DAQC2_calDAC(int addr);
void DAQC2_calADC(int addr);
...
The DIGI (Digital I/O) plate provides 8 digital outputs and 8 digital inputs.
...
```c
void DIGI_setDOUT(int addr, int bit);
void DIGI_clrDOUT(int addr, int bit);
void DIGI_toggleDOUT(int addr, int bit);
```

**Description:**  
Control individual digital output bits (0-7).

**Example:**
```c
DIGI_setDOUT(0, 0);      // Set bit 0 high
DIGI_clrDOUT(0, 1);      // Set bit 1 low
DIGI_toggleDOUT(0, 2);   // Toggle bit 2
```
...
**Description:**  
Read individual digital input bit.

**Example:**
```c
int state = DIGI_getDIN(0, 0);
if (state) {
    printf("Input 0 is HIGH\n");
} else {
    printf("Input 0 is LOW\n");
}
```
